...
//select the whole english prediction for records 
  $pr_query = "select * from GameHistory"; 
...
<?php
//selecting query
$query ="SELECT id, user, Language, initial, YourGuess, OutputLang, MachinePrediction, lastUpdated, GameType, count1, count2, GameResult FROM GameHistory order by lastUpdated desc limit $start_from, $num_per_page"; 
...
//the header of the table: columns
		echo "<table class='table-striped responsive' width='100%'>
				<tr>
        <th>#</th>
					<th>User</th>
					<th>Input</th>
...
//			$MachinePrediction = $row['MachinePrediction'];
			//echo "<input type='hidden' name='id[$a]' value='$id'>";
			echo"<tr>"; //values of each column*row
       echo"<td><b>".$row['id']."</b></td>";
       echo"<td><b>".$row['user']."</b></td>";
       echo"<td><b>".$row['YourGuess']."</b></td>";
      echo"<td><b>".$row['initial']."</b></td>";
     
      echo"<td><b>".$row['OutputLang']."</b></td>";
       echo"<td><b>".$row['MachinePrediction']."</b></td>";
     echo"<td><b>".$row['GameType']."</b></td>";
      echo"<td><b>".$row['count1'].":".$row['count2']."</b></td>";
      if ($row['GameResult'] == "Won")

      echo"<td><b><font color = 'green'>".$row['GameResult']."</font></b></td>";
            else 
            echo"<td><b><font color = 'yellow'>".$row['GameResult']."</font></b></td>";
  ...
  <?php
	for($i=1; $i <= $total_page; $i++){
		...
		echo "<a href='englishPredictionTable.php?page=".$i."'class='button button1' style='padding:15px 32px;'>$i</a>";
	}
?>
</div></div>
